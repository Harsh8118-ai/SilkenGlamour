...
const WebCart = ({ closeCart }) => {
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-50 bg-opacity-70 bg-gray-900">
      <div className="bg-MainBGColorYellow p-6 rounded-lg shadow-lg w-full max-w-xs">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold text-black">Your Cart</h2>
          <button onClick={closeCart} className="text-red-500 text-lg font-bold"> {/* Make sure closeCart works */}
            Close
          </button>
        </div>
        ...
        {cartItems.length === 0 ? (
          ...
          <div>
            {cartItems.map((item) => (
              <div key={item.id} className="flex justify-between items-center mb-4">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-xs text-gray-700">₹{item.price} x {item.quantity}</p>
                </div>
            ...
            <div className="border-t mt-4 pt-4 border-[#CBB59F]">
              <hr />
              <h3 className="text-lg font-bold text-center text-black">Total: ₹{totalPrice}</h3>
            </div>

            {/* Order Now Button */}
            <button
              onClick={handleOrderNow}
              className={`mt-4 font-bold py-2 px-4 w-full rounded ${
                cartItems.length === 0
